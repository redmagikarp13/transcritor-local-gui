...
# Modelos do MLX-Whisper na HuggingFace
MODELS = ["tiny", "base", "small", "medium", "large-v3"]

def get_repo_id(model_size: str) -> str:
    """Retorna o repo_id do HuggingFace para o modelo MLX."""
    # A comunidade MLX hospeda esses repositórios
    return f"mlx-community/whisper-{model_size}-mlx"

def get_downloaded_models() -> list:
    """Retorna uma lista de model_sizes (mlx) que já estão baixados no cache."""
    try:
    ...
    for repo in hf_cache_info.repos:
        ...
        if repo_id.startswith("mlx-community/whisper-") and repo_id.endswith("-mlx"):
            model_size = repo_id.replace("mlx-community/whisper-", "").replace("-mlx", "")
...
def download_model(model_size: str, progress_callback=None):
    ...
    if progress_callback:
        progress_callback(f"Iniciando download de {model_size} (MLX)...\nIsso pode demorar dependendo da sua conexão.")
    try:
        snapshot_download(repo_id=repo_id)
        if progress_callback:
            progress_callback(f"Modelo MLX {model_size} baixado com sucesso!")
        return True
