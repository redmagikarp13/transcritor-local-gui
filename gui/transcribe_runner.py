...
def run_transcription(files, output_dir, config_override=None):
    ...
    print(f"[{len(files)} arquivo(s) na fila para processamento no Apple Silicon (MLX)]", flush=True)
    ...
    for path_str in files:
        ...
        try:
            # Chama a função nova (agora ela carrega o modelo internamente com MLX)
            tr.transcribe_one(path, cfg)
